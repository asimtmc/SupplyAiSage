...
if st.session_state.run_forecast and 'forecasts' in st.session_state and st.session_state.forecasts:
    ...
    if 'clusters' in st.session_state and st.session_state.clusters is not None:
        # Display the comprehensive cluster visualization
        ...
        with col2:
            ...
            if 'recommended_model' in st.session_state.clusters.columns:
                # Get the most common recommended model per cluster
                model_recommendations = st.session_state.clusters.groupby('cluster_name')['recommended_model'].agg(
        ...
        if show_all:
            ...
            available_metrics = [col for col in metric_columns if col in st.session_state.clusters.columns]
            display_columns.extend(available_metrics)
            
            # Create a clean dataframe for display
            sku_clusters = st.session_state.clusters[display_columns].sort_values('cluster_name')
    ...
    if st.button("Prepare Forecast Export"):
        with st.spinner("Preparing forecast data..."):
            ...
            for sku, forecastdata in st.session_state.forecasts.items():
