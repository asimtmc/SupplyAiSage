...
# Set page config
st.set_page_config(
    page_title="New Demand Forecasting",
    page_icon="📈",
    layout="wide"
)
...
# Page title
st.title("New AI-Powered Demand Forecasting")
st.markdown("""
...
if 'new_forecast_periods' not in st.session_state:
    st.session_state.new_forecast_periods = 12  # default 12 months
if 'new_run_forecast' not in st.session_state:
    st.session_state.new_run_forecast = False
if 'new_selected_sku' not in st.session_state:
    st.session_state.new_selected_sku = None
if 'new_selected_skus' not in st.session_state:
    st.session_state.new_selected_skus = []
if 'new_show_all_clusters' not in st.session_state:
    st.session_state.new_show_all_clusters = False
if 'new_selected_models' not in st.session_state:
    st.session_state.new_selected_models = []
if 'new_sku_options' not in st.session_state:
    st.session_state.new_sku_options = []
if 'new_forecast_in_progress' not in st.session_state:
    st.session_state.new_forecast_in_progress = False
if 'new_forecast_progress' not in st.session_state:
    st.session_state.new_forecast_progress = 0
if 'new_forecast_current_sku' not in st.session_state:
    st.session_state.new_forecast_current_sku = ""
if 'new_apply_sense_check' not in st.session_state:
    st.session_state.new_apply_sense_check = True
if 'new_forecasts' not in st.session_state:
    st.session_state.new_forecasts = {}
if 'new_clusters' not in st.session_state:
    st.session_state.new_clusters = None
...
with st.sidebar:
    ...
    if 'sales_data' in st.session_state and st.session_state.sales_data is not None:
        ...
        def update_selected_skus():
            new_selection = st.session_state.new_sku_multiselect
            st.session_state.new_selected_skus = new_selection
            if new_selection:
                st.session_state.new_selected_sku = new_selection[0]  # Set first selected SKU as primary
        ...
        if st.session_state.new_selected_skus:
            default_selection = [sku for sku in st.session_state.new_selected_skus if sku in available_skus]
    ...
    forecast_periods = st.slider(
        ...
        min_value=1,
        max_value=24,
        value=st.session_state.new_forecast_periods,
        step=1
    )
    st.session_state.new_forecast_periods = forecast_periods
    ...
    st.session_state.new_selected_models = models_to_evaluate
    ...
    apply_sense_check = st.toggle(
        ...
        value=st.session_state.new_apply_sense_check,
        key="sidebar_apply_sense_check",
        help="Apply business logic and pattern recognition to ensure realistic forecasts"
    )
    st.session_state.new_apply_sense_check = apply_sense_check

    # Get selected SKUs from session state
    selected_skus_to_forecast = []
    if 'new_selected_skus' in st.session_state and st.session_state.new_selected_skus:
        selected_skus_to_forecast = st.session_state.new_selected_skus
    ...
    if should_show_button:
        ...
        if run_forecast_clicked:
            ...
            if cached_results_available:
                ...
                st.session_state.new_forecasts = st.session_state.new_forecast_cache[cache_key]['forecasts']
                ...
                try:
                    ...
                    with spinner_placeholder:
                        with st.spinner("Building forecast models..."):
                            ...
                            st.session_state.new_forecasts = advanced_generate_forecasts(
                                sales_data=st.session_state.sales_data,
                                cluster_info=st.session_state.new_clusters,
                                forecast_periods=st.session_state.new_forecast_periods,
                                auto_select=True,
                    ...
                    while st.session_state.new_forecast_progress < 1 and st.session_state.new_forecast_current_sku:
                        ...
                        with spinner_placeholder:
                            with st.spinner(f"Processing {st.session_state.new_forecast_current_sku}..."):
                                # Update progress display with more dynamic information
                                status_text.markdown(f"### Processing: **{st.session_state.new_forecast_current_sku}**")
                                progress_percentage = int(st.session_state.new_forecast_progress * 100)
                                progress_details.info(f"Completed: **{progress_percentage}%** | Current SKU: **{st.session_state.new_forecast_current_sku}**")
                    ...
                    if st.session_state.new_forecasts:
                        ...
                        st.session_state.new_forecast_cache[cache_key] = {
                            'forecasts': st.session_state.new_forecasts,
                            'clusters': st.session_state.new_clusters,
...
# Main content
if st.session_state.new_run_forecast and 'new_forecasts' in st.session_state and st.session_state.new_forecasts:
    ...
    with col1:
        ...
        cluster_fig = plot_cluster_summary(st.session_state.new_clusters)
    ...
    with col2:
        ...
        if 'new_clusters' in st.session_state and st.session_state.new_clusters is not None:
            cluster_groups = st.session_state.new_clusters.groupby('cluster_name').size().reset_index()
            ...
            show_all = st.checkbox("Show All SKUs and Their Clusters", value=st.session_state.new_show_all_clusters)
            st.session_state.new_show_all_clusters = show_all

            if show_all:
                sku_clusters = st.session_state.new_clusters[['sku', 'cluster_name']].sort_values('cluster_name')
    ...
    sku_list = list(st.session_state.new_forecasts.keys())

    # Safely get an index for the selected SKU
    if st.session_state.new_selected_sku is None or st.session_state.new_selected_sku not in sku_list:
        default_index = 0
    else:
        try:
            default_index = sku_list.index(st.session_state.new_selected_sku)
    ...
    selected_sku = st.selectbox(
        ...
        options=sku_list,
        index=default_index,
        key="new_forecast_sku_selector"
    )
    st.session_state.new_selected_sku = selected_sku

    # Show forecast details for selected SKU
    if selected_sku:
        forecast_data = st.session_state.new_forecasts[selected_sku]
        ...
        with forecast_tabs[0]:
            ...
            with col_options2:
                ...
                if not show_all_models and available_models:
                    ...
                    for m in st.session_state.new_selected_models:
                        model_upper = m.upper()
            ...
            if show_all_models and available_models:
                ...
                selected_models_for_viz = []
                for model in st.session_state.new_selected_models:
    ...
    if st.button("Prepare Forecast Export", key="new_prepare_forecast_export"):
        with st.spinner("Preparing forecast data..."):
            ...
            st.download_button(
                ...
                data=excel_buffer,
                file_name=f"new_forecasts_{datetime.now().strftime('%Y%m%d')}.xlsx",
    ...
    if st.session_state.new_forecasts:
        ...
        first_sku = list(st.session_state.new_forecasts.keys())[0]
        first_forecast = st.session_state.new_forecasts[first_sku]
        ...
        if 'sales_data' in st.session_state and st.session_state.sales_data is not None:
            ...
            all_skus = sorted(list(st.session_state.new_forecasts.keys()))
            ...
            for sku in table_skus:
                forecast_data_for_sku = st.session_state.new_forecasts[sku]
            ...
            if all_sku_data:
                ...
                with col1:
                    ...
                    st.download_button(
                        ...
                        data=excel_buffer,
                        file_name=f"new_sku_forecast_data_{datetime.now().strftime('%Y%m%d')}.xlsx",
                        mime="application/vnd.ms-excel",
                ...
                with col2:
                    ...
                    st.download_button(
                        ...
                        data=csv_buffer,
                        file_name=f"new_sku_forecast_data_{datetime.now().strftime('%Y%m%d')}.csv",
                        mime="text/csv",
    ...
    if 'sales_data' in st.session_state and st.session_state.sales_data is not None:
        ...
        with col1:
            ...
            if st.session_state.new_selected_sku is None or st.session_state.new_selected_sku not in all_skus:
                default_index = 0
            else:
                try:
                    default_index = all_skus.index(st.session_state.new_selected_sku)
                except (ValueError, IndexError):
                    default_index = 0
            ...
            selected_sku_preview = st.selectbox(
                ...
                options=all_skus,
                index=default_index,
                key="new_selected_sku_preview"
            )

            # Update session state
            st.session_state.new_selected_sku = selected_sku_preview
        ...
        if st.session_state.new_selected_sku:
            st.subheader(f"Historical Sales for {st.session_state.new_selected_sku}")

            # Filter data for the selected SKU
            sku_data = st.session_state.sales_data[st.session_state.sales_data['sku'] == st.session_state.new_selected_sku]
            ...
            if not sku_data.empty:
                ...
                fig = px.line(
                    ...
                    x='date', 
                    y='quantity',
                    title=f'Historical Sales for {st.session_state.new_selected_sku}',
