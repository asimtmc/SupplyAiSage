...
# Set page config
st.set_page_config(
    page_title="Enhanced Demand Forecasting",
    page_icon="📊",
    layout="wide"
)

# Initialize cache for forecast results
if 'enhanced_forecast_cache' not in st.session_state:
    st.session_state.enhanced_forecast_cache = {}

# Flag to track if models are loaded (lazy loading)
if 'enhanced_models_loaded' not in st.session_state:
    st.session_state.enhanced_models_loaded = False
...
# Page title
st.title("Enhanced AI-Powered Demand Forecasting")
st.markdown("""
...
if 'enhanced_forecast_periods' not in st.session_state:
    st.session_state.enhanced_forecast_periods = 12  # default 12 months
if 'enhanced_run_forecast' not in st.session_state:
    st.session_state.enhanced_run_forecast = False
if 'enhanced_selected_sku' not in st.session_state:
    st.session_state.enhanced_selected_sku = None
if 'enhanced_selected_skus' not in st.session_state:
    st.session_state.enhanced_selected_skus = []
if 'enhanced_show_all_clusters' not in st.session_state:
    st.session_state.enhanced_show_all_clusters = False
if 'enhanced_selected_models' not in st.session_state:
    st.session_state.enhanced_selected_models = []
if 'enhanced_sku_options' not in st.session_state:
    st.session_state.enhanced_sku_options = []
if 'enhanced_forecast_in_progress' not in st.session_state:
    st.session_state.enhanced_forecast_in_progress = False
if 'enhanced_forecast_progress' not in st.session_state:
    st.session_state.enhanced_forecast_progress = 0
if 'enhanced_forecast_current_sku' not in st.session_state:
    st.session_state.enhanced_forecast_current_sku = ""
if 'enhanced_apply_sense_check' not in st.session_state:
    st.session_state.enhanced_apply_sense_check = True
if 'enhanced_forecasts' not in st.session_state:
    st.session_state.enhanced_forecasts = {}
if 'enhanced_clusters' not in st.session_state:
    st.session_state.enhanced_clusters = None
if 'enhanced_current_model' not in st.session_state:
    st.session_state.enhanced_current_model = ""
if 'enhanced_log_messages' not in st.session_state:
    st.session_state.enhanced_log_messages = []
...
with st.sidebar:
    ...
    if 'sales_data' in st.session_state and st.session_state.sales_data is not None:
        ...
        def update_selected_skus():
            new_selection = st.session_state.enhanced_sku_multiselect
            st.session_state.enhanced_selected_skus = new_selection
            if new_selection:
                st.session_state.enhanced_selected_sku = new_selection[0]  # Set first selected SKU as primary
        ...
        if st.session_state.enhanced_selected_skus:
            default_selection = [sku for sku in st.session_state.enhanced_selected_skus if sku in available_skus]
    ...
    forecast_periods = st.slider(
        ...
        min_value=1,
        max_value=24,
        value=st.session_state.enhanced_forecast_periods,
        step=1
    )
    st.session_state.enhanced_forecast_periods = forecast_periods
    ...
    # Advanced models including all options from both pages
    ...
    st.session_state.enhanced_selected_models = models_to_evaluate
    ...
    apply_sense_check = st.toggle(
        ...
        value=st.session_state.enhanced_apply_sense_check,
        key="sidebar_apply_sense_check",
        help="Apply business logic and pattern recognition to ensure realistic forecasts"
    )
    st.session_state.enhanced_apply_sense_check = apply_sense_check

    # Get selected SKUs from session state
    selected_skus_to_forecast = []
    if 'enhanced_selected_skus' in st.session_state and st.session_state.enhanced_selected_skus:
        selected_skus_to_forecast = st.session_state.enhanced_selected_skus
    ...
    def forecast_progress_callback(current_index, current_sku, total_skus, message=None, level="info"):
        ...
        st.session_state.enhanced_forecast_progress = progress
        st.session_state.enhanced_forecast_current_sku = current_sku
        ...
        if message:
            ...
            if "model" in message.lower():
                ...
                if len(model_parts) > 1:
                    ...
                    if model_words:
                        ...
                        st.session_state.enhanced_current_model = model_name
            ...
            for keyword in model_keywords:
                if keyword in message.lower():
                    st.session_state.enhanced_current_model = keyword.upper()
                    break

            # Add to log messages
            timestamp = datetime.now().strftime("%H:%M:%S")
            st.session_state.enhanced_log_messages.append({
                "timestamp": timestamp,
                "message": f"[SKU: {current_sku}] {message}",
                "level": level
            })

    # Run forecast button
    forecast_button_text = "Run Enhanced Forecast Analysis"
    ...
    if should_show_button:
        ...
        cached_results_available = cache_key in st.session_state.enhanced_forecast_cache
        ...
        run_forecast_clicked = st.button(
            button_text, 
            key="enhanced_run_forecast_button",
        ...
        if run_forecast_clicked:
            ...
            if cached_results_available:
                ...
                st.session_state.enhanced_forecasts = st.session_state.enhanced_forecast_cache[cache_key]['forecasts']
                st.session_state.enhanced_clusters = st.session_state.enhanced_forecast_cache[cache_key]['clusters']
                st.session_state.enhanced_run_forecast = True
                st.session_state.enhanced_models_loaded = True
                
                # Show success message
                st.success(f"Loaded cached forecasts for {len(st.session_state.enhanced_forecasts)} SKUs!")
                
            else:
                # Set forecast in progress flag
                st.session_state.enhanced_forecast_in_progress = True
                st.session_state.enhanced_forecast_progress = 0
                st.session_state.enhanced_run_forecast = True
                ...
                try:
                    ...
                    with spinner_placeholder:
                        with st.spinner("Clustering SKUs..."):
                            ...
                            st.session_state.enhanced_clusters = cluster_skus(features_df, n_clusters=num_clusters)
                    ...
                    add_log_message(f"Forecast periods: {st.session_state.enhanced_forecast_periods}", "info")
                    add_log_message(f"Human-like sense check: {'Enabled' if st.session_state.enhanced_apply_sense_check else 'Disabled'}", "info")
                    ...
                    with spinner_placeholder:
                        with st.spinner("Building forecast models..."):
                            ...
                            st.session_state.enhanced_forecasts = advanced_generate_forecasts(
                                sales_data=st.session_state.sales_data,
                                cluster_info=st.session_state.enhanced_clusters,
                                forecast_periods=st.session_state.enhanced_forecast_periods,
                                auto_select=True,
                                models_to_evaluate=models_to_evaluate,
                                selected_skus=skus_to_forecast,
                                progress_callback=enhanced_forecast_callback,
                                hyperparameter_tuning=False,
                                apply_sense_check=st.session_state.enhanced_apply_sense_check,
                    ...
                    while st.session_state.enhanced_forecast_progress < 1 and st.session_state.enhanced_forecast_current_sku and (time.time() - start_time < max_wait_time):
                        current_progress = 20 + int(st.session_state.enhanced_forecast_progress * 80)
                        if current_progress > last_progress:
                            progress_bar.progress(current_progress/100)
                            last_progress = current_progress

                        # Get current model and SKU info
                        current_sku = st.session_state.enhanced_forecast_current_sku
                        current_model = getattr(st.session_state, 'enhanced_current_model', 'Unknown')
                        ...
                        with spinner_placeholder:
                            with st.spinner(f"Processing {current_sku}..."):
                                ...
                                progress_percentage = int(st.session_state.enhanced_forecast_progress * 100)
                    ...
                    if time.time() - start_time >= max_wait_time:
                        ...
                        st.session_state.enhanced_forecast_progress = 1
                    ...
                    if st.session_state.enhanced_forecasts:
                        sku_list = sorted(list(st.session_state.enhanced_forecasts.keys()))
                        st.session_state.enhanced_sku_options = sku_list
                        if sku_list and not st.session_state.enhanced_selected_sku in sku_list:
                            st.session_state.enhanced_selected_sku = sku_list[0]

                        # Cache the forecast results for future use
                        cache_key = f"{forecast_scope}_{len(selected_skus_to_forecast)}_{forecast_periods}_{num_clusters}_{'-'.join(models_to_evaluate)}"
                        st.session_state.enhanced_forecast_cache[cache_key] = {
                            'forecasts': st.session_state.enhanced_forecasts,
                    ...
                    num_skus = len(st.session_state.enhanced_forecasts)
                    if num_skus > 0:
...
# Main content
if st.session_state.enhanced_run_forecast and 'enhanced_forecasts' in st.session_state and st.session_state.enhanced_forecasts:
    ...
    with col1:
        ...
        cluster_fig = plot_cluster_summary(st.session_state.enhanced_clusters)
    ...
    with col2:
        ...
        if 'enhanced_clusters' in st.session_state and st.session_state.enhanced_clusters is not None:
            cluster_groups = st.session_state.enhanced_clusters.groupby('cluster_name').size().reset_index()
            ...
            show_all = st.checkbox("Show All SKUs and Their Clusters", value=st.session_state.enhanced_show_all_clusters)
            st.session_state.enhanced_show_all_clusters = show_all

            if show_all:
                sku_clusters = st.session_state.enhanced_clusters[['sku', 'cluster_name']].sort_values('cluster_name')
    ...
    for sku, forecast_data in st.session_state.enhanced_forecasts.items():
    ...
    with process_log_container:
        ...
        if st.session_state.enhanced_log_messages:
            log_html = '<div style="height: 200px; overflow-y: auto; font-family: monospace; font-size: 0.8em; background-color: #f0f0f0; padding: 10px; border-radius: 5px;">'

            for log in st.session_state.enhanced_log_messages[-100:]:  # Show last 100 messages
    ...
    sku_list = list(st.session_state.enhanced_forecasts.keys())

    # Safely get an index for the selected SKU
    if st.session_state.enhanced_selected_sku is None or st.session_state.enhanced_selected_sku not in sku_list:
        default_index = 0
    else:
        try:
            default_index = sku_list.index(st.session_state.enhanced_selected_sku)
    ...
    selected_sku = st.selectbox(
        ...
        options=sku_list,
        index=default_index,
        key="enhanced_forecast_sku_selector"
    )
    st.session_state.enhanced_selected_sku = selected_sku

    # Show forecast details for selected SKU
    if selected_sku:
        forecast_data = st.session_state.enhanced_forecasts[selected_sku]
        ...
        with forecast_tabs[0]:
            ...
            with col_options2:
                ...
                if not show_all_models and available_models:
                    ...
                    for m in st.session_state.enhanced_selected_models:
                        model_upper = m.upper()
            ...
            if show_all_models and available_models:
                ...
                selected_models_for_viz = []
                for model in st.session_state.enhanced_selected_models:
            ...
            with st.expander("Forecast Details", expanded=False):
                if selected_sku and selected_sku in st.session_state.enhanced_forecasts:
    ...
    if st.button("Prepare Forecast Export", key="enhanced_prepare_forecast_export"):
        with st.spinner("Preparing forecast data..."):
            ...
            st.download_button(
                ...
                data=excel_buffer,
                file_name=f"enhanced_forecasts_{datetime.now().strftime('%Y%m%d')}.xlsx",
    ...
    if st.session_state.enhanced_forecasts:
        ...
        first_sku = list(st.session_state.enhanced_forecasts.keys())[0]
        first_forecast = st.session_state.enhanced_forecasts[first_sku]
        ...
        if 'sales_data' in st.session_state and st.session_state.sales_data is not None:
            ...
            all_skus = sorted(list(st.session_state.enhanced_forecasts.keys()))
            ...
            for sku in table_skus:
                forecast_data_for_sku = st.session_state.enhanced_forecasts[sku]
            ...
            if all_sku_data:
                ...
                with col1:
                    ...
                    st.download_button(
                        ...
                        data=excel_buffer,
                        file_name=f"enhanced_sku_forecast_data_{datetime.now().strftime('%Y%m%d')}.xlsx",
                        mime="application/vnd.ms-excel",
                ...
                with col2:
                    ...
                    st.download_button(
                        ...
                        data=csv_buffer,
                        file_name=f"enhanced_sku_forecast_data_{datetime.now().strftime('%Y%m%d')}.csv",
                        mime="text/csv",
    ...
    if 'sales_data' in st.session_state and st.session_state.sales_data is not None:
        ...
        with col1:
            ...
            if st.session_state.enhanced_selected_sku is None or st.session_state.enhanced_selected_sku not in all_skus:
                default_index = 0
            else:
                try:
                    default_index = all_skus.index(st.session_state.enhanced_selected_sku)
                except (ValueError, IndexError):
                    default_index = 0
            ...
            selected_sku_preview = st.selectbox(
                ...
                options=all_skus,
                index=default_index,
                key="enhanced_selected_sku_preview"
            )

            # Update session state
            st.session_state.enhanced_selected_sku = selected_sku_preview
        ...
        if st.session_state.enhanced_selected_sku:
            st.subheader(f"Historical Sales for {st.session_state.enhanced_selected_sku}")

            # Filter data for the selected SKU
            sku_data = st.session_state.sales_data[st.session_state.sales_data['sku'] == st.session_state.enhanced_selected_sku]
            ...
            if not sku_data.empty:
                ...
                fig = px.line(
                    ...
                    x='date', 
                    y='quantity',
                    title=f'Historical Sales for {st.session_state.enhanced_selected_sku}',
